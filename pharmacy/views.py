...
from .forms import MedicineForm, DispensingForm, StockMovementForm, MedicineCategoryForm
...
def pharmacy_dashboard(request):
    ...
    recent_dispensings = Dispensing.objects.select_related('medicine').order_by('-dispensed_at')[:10]
    total_dispensed_today = Dispensing.objects.filter(dispensed_at__date=__import__('datetime').date.today()).aggregate(total=Sum('total_price'))['total'] or 0
...
def category_list(request):
    ...
    return render(request, 'pharmacy/category_list.html', {
